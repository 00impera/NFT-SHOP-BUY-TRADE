...
async function getClaimParams(options) {
  const cc = await (async () => {
    if (options.type === "erc1155") {
      const { getActiveClaimCondition: getActiveClaimCondition2 } = await import("./getActiveClaimCondition-B5Z3IDJK.js");
    ...
    if (options.type === "erc721") {
      if (options.singlePhaseDrop) {
        const { claimCondition } = await import("./claimCondition-NNI64GIQ.js");
        return await claimCondition({
          contract: options.contract
        });
      }
      const { getActiveClaimCondition: getActiveClaimCondition2 } = await import("./getActiveClaimCondition-WE3NB7CE.js");
      return await getActiveClaimCondition2({
        contract: options.contract
      });
    }
    ...
    const { getActiveClaimCondition } = await import("./getActiveClaimCondition-6D4LON5C.js");
  ...
  const allowlistProof = await (async () => {
    ...
    const { fetchProofsForClaimer } = await import("./fetch-proofs-for-claimers-BCCJIIDL.js");
...
//# sourceMappingURL=chunk-63C7YNCC.js.map
