...
import {
  ...
  encode as encode2,
  format
} from "./chunk-HXOGVIYT.js";
import {
  getContractEvents
} from "./chunk-DDA36Z3A.js";
import {
  decimals
} from "./chunk-DOD4YXVY.js";
import {
...
import {
  ...
  toBytes as toBytes3,
  toHex as toHex2
} from "./chunk-ATIDAIW6.js";
import {
  Solidity_exports
} from "./chunk-W4UZWQ2D.js";
import {
  eth_call
...
import {
  extractError,
  resolveContractAbi
} from "./chunk-SP5BNSKI.js";
import {
  formatAbiItem
} from "./chunk-BREECRX7.js";
import {
  encode
} from "./chunk-7WSH7PLI.js";
import {
...
import {
  Buy_exports,
  Sell_exports
} from "./chunk-AYD5FKXU.js";
import {
  getCachedChain
} from "./chunk-EWBX2J76.js";
import {
  status
} from "./chunk-TT4X7S7V.js";
import {
  getClientFetch
} from "./chunk-FZ4YV5XS.js";
import {
  getThirdwebDomains
...
function encode3(abiError, ...args) {
...
function encode4(abiEvent, ...[args]) {
  let topics = [];
...
function fromString3(value) {
  return from10(fromString2(value));
}
...
function toString4(value) {
  return toString2(toBytes5(value));
}
...
function fromCreate2(options) {
  ...
  return from5(slice(keccak256(concat2(fromHex("0xff"), from19, salt, bytecodeHash), { as: "Hex" }), 12));
}
...
function verify2(options) {
  ...
  return secp256k1.verify(signature, from2(payload), toBytes3(publicKey), ...hash3 ? [{ prehash: true, lowS: true }] : []);
}
...
function assert2(envelope) {
  ...
  if (blobVersionedHashes) {
    ...
    for (const hash3 of blobVersionedHashes) {
      const size4 = size(hash3);
      const version = toNumber(slice(hash3, 0, 1));
      if (size4 !== 32)
...
function verify4(options) {
  const { challenge, hash: hash3 = true, metadata, publicKey, signature } = options;
    ...
    const swapRoute = {
      ...
      swapDetails: {
        estimated: {
          ...
          feesUSDCents: 0,
          fromAmountUSDCents: Number(Value_exports.format(quote.originAmount, firstStep.originToken.decimals)) * (firstStep.originToken.prices.USD || 0) * 100,
          gasCostUSDCents: 0,
          slippageBPS: 0,
          toAmountMinUSDCents: Number(Value_exports.format(quote.destinationAmount, firstStep.destinationToken.decimals)) * (firstStep.destinationToken.prices.USD || 0) * 100,
          toAmountUSDCents: Number(Value_exports.format(quote.destinationAmount, firstStep.destinationToken.decimals)) * (firstStep.destinationToken.prices.USD || 0) * 100
        },
...
async function simulateTransaction(options) {
  var _a;
  const [data, to2, accessList, value] = await Promise.all([
    encode(options.transaction),
    resolvePromisedValue(options.transaction.to),
    resolvePromisedValue(options.transaction.accessList),
    resolvePromisedValue(options.transaction.value)
  ]);
  const from19 = options.from ?? ((_a = options.account) == null ? void 0 : _a.address) ?? void 0;
  const serializedTx = formatTransactionRequest({
    accessList,
    data,
    from: from19 ? getAddress(from19) : void 0,
    to: to2 ? getAddress(to2) : void 0,
    value
  });
...
@noble/curves/esm/p256.js:
  (*! noble-curves - MIT License (c) 2022 Paul Miller (paulmillr.com) *)
*/
//# sourceMappingURL=chunk-E5DM4K27.js.map
